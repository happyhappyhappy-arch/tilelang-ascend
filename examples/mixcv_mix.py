...
def minicv(M, N, K, block_M, block_N, block_K):
    ...
    def minicv(
        ...
        with T.Kernel(m_num * n_num, is_npu=True) as (cid, vid):
            blockx = cid // n_num # 0, 1
            bx = blockx * block_M # [0,64]
            blocky = cid % n_num #0,1,2,3
            by = blocky * block_N # [0,64,64*2,64*3]
            D_BUF = T.alloc_shared((block_M, block_N), inner_dtype)
            for k in T.Pipelined(T.ceildiv(K, block_K)//multi_buffer, num_stages=multi_buffer):
                ...
                workspace = T.alloc_workspace((block_M, block_K), dtype, multi_buffer=multi_buffer)
