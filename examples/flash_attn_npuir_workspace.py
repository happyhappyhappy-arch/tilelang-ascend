...
def online_flash_attention(block_M, block_N, dtype="float16", accum_dtype="float32"):
    ...
    def flash_attention(
        ...
        with T.Kernel(block_num, is_npu=True) as (cid, vid):
            ...
            Workspace1 = T.alloc_workspace((block_num, block_m, block_n), accum_dtype, multi_buffer = multi_buffer)
            Workspace2 = T.alloc_workspace((block_num, block_m, block_n), dtype, multi_buffer = multi_buffer)
            Workspace3 = T.alloc_workspace((block_num, block_m, dim), accum_dtype, multi_buffer = multi_buffer)
            ...
            for k in T.Pipelined(T.ceildiv(seq_len, block_n)//multi_buffer, num_stages=multi_buffer):
                ...
                # vec
                T.copy(Workspace1[cid, vid * block_m // 2, 0], scores_ub, size=[block_m // 2, block_n])
                ...
                T.copy(scores_cast, Workspace2[cid, vid * block_m // 2, 0], size=[block_m // 2, block_n])
                ...
                # vec
                T.copy(Workspace3[cid, vid * block_m // 2, 0], acc_o_ub, size=[block_m // 2, dim])
...
def main():
    ...
    block_num = seq_len // 64
    w1 = torch.randn((2, block_num, 64, 64), dtype=torch.float32).npu()
    w2 = torch.randn((2, block_num, 64, 64), dtype=torch.float16).npu()
    w3 = torch.randn((2, block_num, 64, 128), dtype=torch.float16).npu()
